package com.ruoyi.union.tbk.module;
...
public class TbkModule {
    ...
    public Map<String, Object> getTpwd(Map<String, String> params) {
        return getTpwd(params, taobaoClient);
    }

    /**
     * 获取淘口令（使用指定的TaobaoClient）
     * 该方法用于根据传入的参数和自定义客户端生成淘宝客淘口令。
     * @param params 参数，包含生成淘口令所需的信息，如标题、链接、logo 等
     * @param client 自定义的TaobaoClient（用于代理配置）
     * @return 包含淘口令数据的 Map 对象
     */
    public Map<String, Object> getTpwd(Map<String, String> params, TaobaoClient client) {
        // 记录请求参数，方便调试和监控
        log("请求参数：{}", params);
        // 创建淘宝客淘口令生成请求对象
        TbkTpwdCreateRequest req = new TbkTpwdCreateRequest();
        // 如果参数中包含text，则设置到请求对象中
        if (params.containsKey("text")) {
            req.setText(params.get("text"));
        }
        // 如果参数中包含title，则设置到请求对象中（兼容旧参数）
     ...
     */
    public Map<String, Object> getShortUrl(Map<String, String> params) {
        // 记录请求参数，方便调试和监控
        log("请求参数：{}", params);
        // 从参数中获取需要转换的 URL
     ...
     */
    public <T extends TaobaoResponse> T execute(Map<String, String> params) {
        return execute(params, taobaoClient);
    }

    /**
     * 淘宝客通用接口（使用自定义TaobaoClient）
     * 该方法是一个通用的淘宝客接口，可根据传入的 API 方法名和参数调用相应的淘宝客 API
     * @param <T>    泛型类型，继承自 TaobaoResponse
     * @param params 参数，包含 API 方法名和调用该 API 所需的其他参数
     * @param customClient 自定义的TaobaoClient实例（如代理的客户端）
     * @return 淘宝客 API 的响应对象
     */
    public <T extends TaobaoResponse> T execute(Map<String, String> params, TaobaoClient customClient) {
        // 记录请求参数，方便调试和监控
        log("请求参数：{}", params);
        // 从参数中获取 API 方法名
        String apiMethodName = params.get("apiMethodName");
        // 如果 API 方法名为空
        if (StrUtil.isBlank(apiMethodName)) throw new ServiceException("参数【apiMethodName】方法名称为空！");
        ...
        // 如果请求类为空
        if (requestClass == null) throw new ServiceException(apiMethodName + "方法未集成！");
        // 使用反射创建请求对象
        ...
        T rsp;
        try {
            // 执行请求并获取响应，使用传入的自定义客户端
            rsp = customClient.execute(request);
        } catch (ApiException e) {
            // 若出现 API 异常
            throw new ServiceException(e.getErrMsg());
        }
    ...
    private void log(String tag, Object object) {
        ...
        if (tbkConfig.isDebug()) log.info(tag, JSONUtil.formatJsonStr(JSONUtil.toJsonStr(object)));
    }
