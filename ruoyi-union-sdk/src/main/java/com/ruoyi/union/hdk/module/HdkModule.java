package com.ruoyi.union.hdk.module;
...
public class HdkModule {
     ...
     */
    public JSON execute(Map<String, String> params) {
        // 记录请求参数，方便调试和监控
        if (config.isDebug()) log.info("请求参数：{}", JSONUtil.formatJsonStr(JSONUtil.toJsonStr(params)));
        // 从参数中获取 API 方法名
        ...
        try {
            ...
            // 记录原始响应报文
            if (config.isDebug()) log.info("返回原报文：{}", result);
            // 将 Unicode 编码的响应结果转换为普通字符串
            result = UnicodeUtil.toString(result);
            // 将响应结果解析为 JSON 对象
            resultJson = JSONUtil.parse(result);
            // 记录格式化后的响应报文
            if (config.isDebug()) log.info("返回报文：{}", JSONUtil.formatJsonStr(JSONUtil.toJsonStr(resultJson)));
        } catch (Exception e) {
